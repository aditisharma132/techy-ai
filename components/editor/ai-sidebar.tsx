...
export function AiSidebar({ isOpen, onClose, roomId, projectId }: AiSidebarProps) {
  ...
  const handleRunTerminal = useCallback(
    (status: string, output: unknown) => {
      ...
      const content = isSuccess
        ...
        : "Techy AI encountered an error. Please try again."
  ...
  const handleSend = useCallback(async () => {
    ...
    createFeedMessage(FEED_ID, {
      text: "Techy AI is analyzing your request…",
      status: "start",
    }).catch(() => {})

    setStatusText("Techy AI is analyzing your request…")
    ...
    try {
      ...
      createFeedMessage(CHAT_FEED_ID, {
        sender: "Techy AI",
        role: "assistant",
        content: "Failed to reach Techy AI. Please try again.",
        timestamp: new Date().toISOString(),
      }).catch(() => {})

      createFeedMessage(FEED_ID, {
        text: "Techy AI encountered an error.",
        status: "error",
      }).catch(() => {})
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-3 border-b border-border-default px-5 py-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-text-muted">Collaborate with Techy AI</p>
        </div>
        {isLoading && (
      ...
      <Tabs defaultValue="architect" className="flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        {/* Specs Tab */}
        <TabsContent value="specs" className="min-h-0 flex-1 overflow-hidden data-[state=active]:flex data-[state=active]:flex-col">
